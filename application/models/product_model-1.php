...
class Product_model extends My_Model
{
	...
	public function view_product_details($condition = ''){
		...
		pr.id as reserve_id, pr.dateAdded as reservedDate, pr.sold_admin_id from ".PRODUCT." p 
	...
	public function view_product_details1($condition = ''){
		...
		pp.imgtitle,pp.imgPriority,pp.product_image,
		pat.attr_name,
		pats.subattr_name,
		sa.user_name as sold_admin_name,
		pr.property_type, pr.property_sub_type,
		...
		LEFT JOIN ".CITY." ca on ca.id=pa.city
		LEFT JOIN ".STATE_TAX." st on st.id=pa.state
		LEFT JOIN ".PRODUCT." pr on pr.id=p.property_id
		LEFT JOIN ".PRODUCT_ATTRIBUTE." pat on pat.id=pr.property_type
		LEFT JOIN ".PRODUCT_SUBATTRIBUTE." pats on pats.id = pr.property_sub_type
		LEFT JOIN ".USERS." sa on sa.id = p.sold_admin_id
		LEFT JOIN ".STATUS." ps on ps.reserved_id = p.id
		LEFT JOIN ".PRODUCT_PHOTOS." pp on pp.property_id=p.property_id ".$condition;
	...
	public function view_product_details2($condition = ''){
		...
		$this->db->join(PRODUCT_ADDRESS.' as pa','pa.property_id=p.property_id');
		$this->db->join(CITY.' as ca','ca.id=pa.city');
		$this->db->join(STATE_TAX.' as st','st.id=pa.state');
		$this->db->join(PRODUCT.' as pr','pr.id=p.property_id');
		$this->db->join(PRODUCT_ATTRIBUTE.' as pat','pat.id=pr.property_type');
		$this->db->join(PRODUCT_SUBATTRIBUTE.' as pats','pats.id=pr.property_sub_type');
		$this->db->join(USERS.' as sa','sa.id=p.sold_admin_id');
		$this->db->join(STATUS.' as ps','ps.reserved_id=p.id');
		$this->db->join(PRODUCT_PHOTOS.' as pp','pp.property_id=p.property_id');
		$this->db->where($condition);
	...
	function get_product_images($product_id_list = array(),$isLimitCond = '')
	{
...
*/		
	...
	public function get_product_details_Cat($condition='',$whereCantOrder='',$searchPerPage='',$paginationNo='')
		...
		//print_r($condition);
			...
			}else if($whereCantOrder=='pricedesc'){
   ...
   public function saveResevedSettings(){
    ...
    public function saveSoldSettings(){
		...
		//print_r($getSoldSettingsDetails->result()); die;
