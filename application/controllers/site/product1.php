...
//redirect(base_url());		
	...
	public function brochure_form(){
	
		$this->data['heading'] = 'View Brochure';
			
			$this->data['prdid'] = $prdid = $this->uri->segment(2,0);
			
			$condition	=	array('id' => $prdid);
			$brochureDetails = $this->product_model->get_all_details(BROCHURE,$condition);
			
			$productDetails = $this->product_model->get_all_details(PRODUCT,array('id'=> $brochureDetails->row()->prop_row_id));
			...
			//echo '<pre>'; print_r($propImage->result());die;
...
<tr><td colspan="3" style="font-family: Arial,Helvetica,sans-serif; text-align: center; font-weight: bold; font-size: 24px; background:  #000; color: rgb(255, 255, 255); text-transform: capitalize; border-radius: 5px; padding:15px;">$'.$brochureDetails->row()->sale_price.'</td></tr>
...
<table style="width: 100%; margin: 0px; border-left: 1px solid rgb(136, 136, 136); border-bottom: 1px solid rgb(136, 136, 136); border-right: 1px solid rgb(136, 136, 136); font-family: Arial,Helvetica,sans-serif; font-weight: bold; font-size: 17px; text-align: left; padding: 14px 10px;">
	...
	$this->load->view('site/product/view_brochure',$this->data);
...
<table style="width: 100%; margin: 0px; border-left: 1px solid rgb(136, 136, 136); border-bottom: 1px solid rgb(136, 136, 136); border-right: 1px solid rgb(136, 136, 136); font-family: Arial,Helvetica,sans-serif; font-weight: bold; font-size: 17px; text-align: left; padding: 14px 10px;">
	...
	public function recentSoldSet(){
		$this->product_model->saveSoldSettings();
		echo 'Siva';die;
	}	
	
	public function tstmail(){
		//$to_mail = 'tc@returnonrentals.com';
		$to_mail = 'jayaprakash@casperon.in';
		$email_values = array('mail_type'=>'html',
							'from_mail_id'=>'jadepaiva1@gmail.com',
							'mail_name'=>'Developer',
							'to_mail_id'=>$to_mail,
							'subject_message'=>'test mail notification',
							'body_messages'=>'test mail notification from developer through site'
							);
		$email_send_to_common = $this->product_model->common_email_send($email_values);
	}	
