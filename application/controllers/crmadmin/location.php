...
class Location extends MY_Controller {
     ...
     */
   	public function index(){	
		if ($this->checkLogin('CA') == ''){
			...
			redirect('crmadmin/location/display_user_list');
		}
	}
	...
	public function display_location_list(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/location/display_location',$this->data);
		}
	}
	...
	public function display_country_list(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/location/display_country',$this->data);
		}
	}
	...
	public function display_user_dashboard(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/location/display_location_dashboard',$this->data);
		}
	}
	...
	public function add_location_form(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/location/add_location',$this->data);
		}
	}
	/**
	 * 
		...
		if ($this->checkLogin('CA') == ''){
			...
			if ($location_id == ''){
				...
				if ($duplicate_name->num_rows() > 0){
					...
					redirect('crmadmin/location/add_location_form');
				}
			}
...
//				$this->location_model->updateCurrencyDetails($excludeArr,$dataArr,$condition);
			...
			redirect('crmadmin/location/display_location_list');
		}
	}
	...
	public function edit_location_form(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($this->data['location_details']->num_rows() == 1){
				$this->load->view('crmadmin/location/edit_location',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
	...
	public function view_location(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->data['location_details'] = $this->location_model->get_all_details(COUNTRY_LIST,$condition);
			if ($this->data['location_details']->num_rows() == 1){
				$this->load->view('crmadmin/location/view_location',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
	...
	public function display_tax_statelist(){ 
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/location/display_tax',$this->data);
		}
	}
	...
	public function add_tax_form(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/location/add_tax',$this->data);
		}
	}
		...
		if ($this->checkLogin('CA') == ''){
			...
			if ($tax_id == ''){
				...
				if ($duplicate_name->num_rows() > 0){
					...
					redirect('crmadmin/location/add_tax_form');
				}
			}
			...
			//redirect('crmadmin/location/display_tax_list');
	...
	public function edit_tax_form(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($this->data['tax_details']->num_rows() == 1){
				$this->load->view('crmadmin/location/edit_tax',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
	...
	public function view_tax(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->data['CountryList'] = $this->location_model->get_all_details(COUNTRY_LIST,$condition1);
			if ($this->data['tax_details']->num_rows() == 1){
				$this->load->view('crmadmin/location/view_tax',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
...
/* Location: ./application/controllers/crmadmin/location.php */
