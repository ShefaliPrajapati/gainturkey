...
class Commission extends MY_Controller {
     ...
     */
   	public function index(){	
		if ($this->checkLogin('CA') == ''){
			...
			redirect('crmadmin/commission/display_commission_lists');
		}
	}
	...
	public function display_commission_lists(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/commission/display_commission_lists',$this->data);
		}
	}
	...
	public function view_paid_details(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/commission/view_paid_details',$this->data);
		}
	}
	...
	public function add_pay_form(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($this->data['sellerDetails']->num_rows()==1){
				...
				$this->load->view('crmadmin/commission/add_vendor_payment',$this->data);
			}else {
				show_404();
			}
		}
...
/*			$balance = $this->input->post('balance_due');
			...
			if ($amount>$balance){
				...
				if ($duplicateCheck->num_rows()>0){
					...
					redirect('crmadmin/commission/view_paid_details/'.$this->input->post('vendor_id'));
				}
			}
*/		
			...
			if ($amount>$balance){
				...
				$this->data['admin_id'] = $this->encrypt->encode($this->checkLogin('CA'), $key);
				$this->data['seller_id'] = $this->encrypt->encode($seller_id, $key);
				$this->data['paypal_email'] = $this->input->post('paypal_email');
				$this->load->view('crmadmin/commission/paypal_form',$this->data);
			}
		}
	}
	...
	public function display_payment_success(){
		if ($this->checkLogin('CA')!=''){
			...
			if ($msg == 'success'){
				...
				if ($admin_id == $this->checkLogin('CA')){
					$dataArr = array(
						'status'			=>	'success',
					);
					...
					$this->load->view('crmadmin/commission/payment_success',$this->data);
				}else {
					show_404();
				}
			}
	...
	public function display_payment_failed(){
		if ($this->checkLogin('CA')!=''){
			...
			$this->load->view('crmadmin/commission/payment_failed',$this->data);
		}else {
			show_404();
		}
	}
...
/* Location: ./application/controllers/crmadmin/commission.php */