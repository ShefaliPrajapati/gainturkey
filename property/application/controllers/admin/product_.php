...
class Product extends MY_Controller {
    ...
    public function display_product_list() {
        if ($this->checkLogin('A') == '') {
            ...
            if ($this->session->userdata('fc_session_admin_name') == $this->config->item('admin_name') || $this->session->userdata('fc_session_admin_name') == 'tcadmin') {
                $this->data['productList'] = $this->product_model->view_product_details(' group by p.id order by p.created desc ');
            } else {
                $this->data['productList'] = $this->product_model->view_product_details(' where p.user_id=' . $this->checkLogin('A') . ' group by p.id order by p.created desc ');
            }
...
<body style="background:#FFFFFF; width:100%; margin:0; padding:0;">
   ...
   <div style="margin:0px auto; width:64%;">
    ...
    public function insertEditProduct() {
        //echo $this->session->userdata('fc_session_admin_mode');
        ...
        if ($this->checkLogin('A') == '')
            ...
            //echo $property_status; die;
            if ($property_id == '') {
                if ($admin_mode == 'fc_subadmin') {
                    $property_status = 'Staging';
                } else {
            ...
            if ($property_id == '') {
                ...
                if ($property_id != '') {
                    ...
                    if ($old_product_details->row()->property_status != 'Staging') {
                        if ($af_row > 0) {
                            if ($admin_mode == 'fc_subadmin') {
                                $property_status = 'Staging';
                            } else {
                        ...
                        $property_status = $old_product_details->row()->property_status;
                    }
                }

                $this->product_model->update_details(PRODUCT, array('property_status' => $property_status, 'modified' => mdate($datestring, $time)), array('id' => $property_id));
