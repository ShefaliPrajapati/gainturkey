...
class Video extends MY_Controller {
     ...
     */
   	public function index(){	
		if ($this->checkLogin('A') == ''){
			...
			redirect('admin/video/display_video_list');
		}
	}
	...
	public function display_video_list(){
		if ($this->checkLogin('A') == ''){
			...
			$this->load->view('admin/video/display_videolist',$this->data);
		}
	}
	...
	public function display_user_dashboard(){
		if ($this->checkLogin('A') == ''){
			...
			$this->load->view('admin/video/display_video_dashboard',$this->data);
		}
	}
	...
	public function add_video_form(){
		if ($this->checkLogin('A') == ''){
			...
			$this->load->view('admin/video/add_video',$this->data);
		}
	}
		...
		if ($this->checkLogin('A') == ''){
			...
			if ($video_id == ''){
				...
				if ($duplicate_name->num_rows() > 0){
					...
					redirect('admin/video/add_video_form');
				}
			}
	...
	public function edit_video_form(){
		if ($this->checkLogin('A') == ''){
			...
			if ($this->data['video_details']->num_rows() == 1){
				$this->load->view('admin/video/edit_video',$this->data);
			}else {
				redirect('admin_ror');
			}
		}
	...
	public function view_video(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['video_details'] = $this->video_model->get_all_details(VIDEO,$condition);
			if ($this->data['video_details']->num_rows() == 1){
				$this->load->view('admin/video/view_video',$this->data);
			}else {
				redirect('admin_ror');
			}
		}
