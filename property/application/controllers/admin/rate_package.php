...
class Rate_package extends MY_Controller {
     ...
     */
   	...
   	public function index(){	
		if ($this->checkLogin('A') == ''){
			...
			redirect('admin/rate_package/display_user_list');
		}
	}
	...
	public function display_package_list(){
		if ($this->checkLogin('A') == ''){
			...
			$this->load->view('admin/rate_package/display_package',$this->data);
		}
	}
	...
	public function display_package_dashboard(){
		if ($this->checkLogin('A') == ''){
			...
			$this->load->view('admin/rate_package/display_package_dashboard',$this->data);
		}
	}
	...
	public function add_package_form(){
		if ($this->checkLogin('A') == ''){
			...
			$this->load->view('admin/rate_package/add_package',$this->data);
		}
	}
	/**
	 * 
		...
		if ($this->checkLogin('A') == ''){
			...
			if ($package_id == ''){
				...
				if ($duplicate_name->num_rows() > 0){
					...
					redirect('admin/rate_package/add_package_form');
				}
			}
			...
			redirect('admin/rate_package/display_package_list');
		}
	}
	...
	public function edit_package_form(){
		if ($this->checkLogin('A') == ''){
			...
			if ($this->data['package_details']->num_rows() == 1){
				$this->load->view('admin/rate_package/edit_package',$this->data);
			}else {
				redirect('admin_ror');
			}
		}
	...
	public function view_package(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['package_details'] = $this->product_model->get_all_details(PRODUCT_RATE_PACKAGE,$condition);
			if ($this->data['package_details']->num_rows() == 1){
				$this->load->view('admin/rate_package/view_package',$this->data);
			}else {
				redirect('admin_ror');
			}
		}
...
/* Rental Rate Package: ./application/controllers/admin/rate_package.php */