...
//						$condition = " where p.category_id like '".$cat.",%' AND p.status = 'Publish' AND p.id != '".$pid."' OR p.category_id like '%,".$cat."' AND p.status = 'Publish' AND p.id != '".$pid."' OR p.category_id like '%,".$cat.",%' AND p.status = 'Publish' AND p.id != '".$pid."' OR p.category_id='".$cat."' AND p.status = 'Publish' AND p.id != '".$pid."'";
	...
	public function display_product_detail($seourl)
		...
		if($this->checkLogin('U')=='')
			...
			{
			$where = array('property_status !='=>'Staging','id'=>$seourl);	
	...
	public function display_all_product()
		...
		{	
        	...
        	$this->data['PropertyList'].='<div class="img_content"><a href="'.base_url().'Property/'.$row->id.'"><img src="images/product/';
            ...
            <p><b class="doller_user">$</b>'.number_format($row->event_price,0).'<span class="number"  style="display:none">'.$row->event_price.'</span></p><span class="name_con">'.$row->cityName.',<span class="name">'.$row->stateName.'</span></span>
            <div class="clear"></div>
            ...
            <div class="sub_title_list"><a href="'.base_url().'Property/'.$row->id.'">'.$row->bedrooms.' Bedrooms + '.$row->baths.' Bathrooms </a></div>
        	</li>';
    	}
	...
	function reservation_form($seourl)
	{
		if($this->checkLogin('U')=='')
			...
			if($this->data['productDetails']->row()->property_status == 'Active')
				{
					$userId = $this->checkLogin('U');
					$this->product_model->update_details(USERS,array('reservation' => 'Yes', 'property_id' => $seourl),array('id' => $userId)); 
			$this->load->model('admin_model');
			$this->data['admin_settings'] = $result = $this->admin_model->getAdminSettings();
			...
			if($_SESSION['reservation'] < time())
				{
					...
					$this->product_model->update_details(USERS,array('reservation' => 'No'),array('id' => $user));
	...
	function ReservationForm_Submit()
		{
			...
			$this->setErrorMessage('success','Congratulations! You have successful Reserved this property! Please have the property specialist print you out a copy of your Property Reservation Confirmation Hot Sheet. Please bring this Confirmation Hot Sheet with you to your closing at this event.');
...
//redirect(base_url());		
	...
	public function changetoActive()
		{	
			...
			$this->product_model->update_details(PRODUCT,$dataArr,$condition);
			$this->product_model->update_details(USERS,array('reservation'=>'No','property_id'=>0),array('id'=>$userID));
			unset($_SESSION['differenceTime']);
			unset($_SESSION['endtimer']);
			unset($_SESSION['reservation']); 
			$this->product_model->saveResevedSettings();
		...
		public function changereservationStatus() 
			{
				...
				if($this->checkLogin('U') != '')
					...
					//$reservedID = $this->product_model->getResevationSettingsOnce();
			...
			if($_SESSION['sCheckTimeReser'] < 4) {
			$_SESSION['sCheckTimeReser'] = $_SESSION['sCheckTimeReser']+1;
				//if(count($reservedID) > 0){
					// foreach($reservedID as $reservedProp){
					foreach($reservedID as $datas){
						$reservedimage = array_filter(explode('#',$datas));
					 
						$prodDisp.= '<div class="innerfade front_popup_step1" id="fadein-'.$reservedimage[0].'">';
						...
						$prodDisp.= '<img src="'.$getimg.'" width="95" height="95" /><p><em>Property id : '.$reservedimage[0].'</em><strong> is</strong> <span> reserved</span></p></div>';
			...
			$_SESSION['sCheckTimeSold'] = $_SESSION['sCheckTimeSold']+1;
				//if(count($reservedID) > 0){
					...
					foreach($soldID as $datad){
						...
						$prodDisp.= '<div class="innerfade front_popup_step2" id="fadein-'.$soldimage[0].'">';
						...
						$prodDisp.= '<img src="'.$getimg.'" width="95" height="95" /><p><em>Property id : '.$soldimage[0].'</em><strong> is</strong> <span> sold</span></p></div>';
