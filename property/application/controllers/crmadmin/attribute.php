...
class Attribute extends MY_Controller {
	...
	function __construct(){
		...
		if ($this->checkPrivileges('code',$this->privStatus) == FALSE){
			redirect('deals_crm');
		}
    }
     ...
     */
   	public function index(){	
		if ($this->checkLogin('CA') == ''){
			...
			redirect('crmadmin/attribute/display_attribute_list');
		}
	}
	...
	public function display_attribute_list(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/attribute/display_attribute_list',$this->data);
		}
	}
	...
	public function display_list_values(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/attribute/display_list_values',$this->data);
		}
	}
	...
	public function add_attribute_form(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/attribute/add_attribute',$this->data);
		}
	}
	...
	public function insertAttribute(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($duplicate_name->num_rows() > 0){
				...
				redirect('crmadmin/attribute/add_attribute_form/');
			}
	...
	public function edit_attribute_form(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($this->data['attribute_details']->num_rows() == 1){
				$this->load->view('crmadmin/attribute/edit_attribute',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
	...
	public function edit_list_value_form(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($this->data['list_value_details']->num_rows() == 1){
				...
				$this->load->view('crmadmin/attribute/edit_list_value',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
	...
	public function view_attribute(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($this->data['attribute_details']->num_rows() == 1){
				$this->load->view('crmadmin/attribute/view_attribute',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
	...
	public function delete_list_value(){
		if ($this->checkLogin('CA') == ''){
			...
			redirect('crmadmin/attribute/display_list_values');
		}
	}
	...
	public function change_attribute_status_global(){
		...
		if($this->input->post('checkboxID')!=''){
			...
			if($this->input->post('checkboxID')=='0'){
				redirect('crmadmin/attribute/add_attribute_form/0');
			}else{
				redirect('crmadmin/attribute/add_attribute_form/'.$this->input->post('checkboxID'));			
			}
	...
	public function add_list_value_form(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->data['list_details'] = $this->attribute_model->get_all_details(ATTRIBUTE,array('status'=>'Active'));
			$this->load->view('crmadmin/attribute/add_list_value',$this->data);
		}
	}
