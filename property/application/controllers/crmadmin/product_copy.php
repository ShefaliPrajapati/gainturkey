...
<body style="background:#FFFFFF; width:100%; margin:0; padding:0;">
	...
	public function insertEditdocumentModule(){
		...
		//SIGNTEMPLATE
			if ($this->checkLogin('CA') == '')
				...
				//echo '<br>'.$filename;
				$filename = $filename.'-'.$property_id.'-'.$reserve_id.'-'.$user_id.'-'.$rental_id;
		 		//echo '<br>'.$filename;
			...
			if(move_uploaded_file($_FILES['upload_pdf']['tmp_name'], $pdfDirectory.$filename)) {
				...
				for($i=0;$i<$num_pag;$i++){
					...
					$thumb = basename( $filename.'-'.$i, ".pdf").".png";
					//echo '<br>'.$thumb;
					exec("convert \"{$pdfWithPath}[".$i."]\" -colorspace RGB -geometry 2550 $thumbDirectory$thumb");
					$imgArr[] = $thumb;
				}
				...
				//exec("composite -compose bumpmap  -tile ".$logoWithPath." \ \"{$pdfWithPath}[0]\" $thumbDirectory$thumb");
				//exec("composite -compose bumpmap  -tile ".$logoWithPath." \ \"{$pdfWithPath}[1]\" $thumbDirectory$thumb1");
				...
				for($i=0;$i<2;$i++){
							if($i==1){
								$config['source_image']    = $thumbDirectory.$thumb;
								$config['wm_text'] = 'Teach This Program';
								$config['wm_type'] = 'text';
								...
								$config['source_image']    = $thumbDirectory.$thumb1;
								$config['wm_text'] = 'Teach This Program';
								$config['wm_type'] = 'text';
								$config['wm_font_path'] = './GILSANUB.TTF';
								$config['wm_font_size']    = '22';
				...
				$dataArr = array('rental_id'=>$rental_id,'prop_address'=>$prop_address,'upload_images'=>$newImgName,'reserve_id'=>$reserve_id,'property_id'=>$property_id,'user_id'=>$user_id,$agree_module=>$agree_module);
						...
						//redirect('crmadmin/product/display_product_list/'.$uri_val);
					
					
					
					/*$dataArr = array('rental_id'=>$rental_id,'prop_address'=>$prop_address,'description'=>$description,'reserve_id'=>$reserve_id,'property_id'=>$property_id,'user_id'=>$user_id,$agree_module=>$agree_module);
					...
					if($sign_id == 0){
						...
						$descripe = str_replace("'.base_url().'", base_url(), $this->input->post('description'));
				
						$content_new = str_replace("{","'.",addslashes($this->input->post('description')));
						$descripe = str_replace("}",".'",$content_new);
						$config = "<?php \$message .= '";
						$config .= "$descripe";
						$config .= "';  ?>";
						$file = 'signtemplate/sign_'.$last_id.'_'.$reserve_id.'_'.$property_id.'_'.$user_id.'_'.$agree_module.'.php';
						...
						$config .= "$descripe";
						$config .= "';  ?>";
						$file = 'signtemplate/sign_'.$sign_id.'_'.$reserve_id.'_'.$property_id.'_'.$user_id.'_'.$agree_module.'.php';
						file_put_contents($file, $config);
						$this->setErrorMessage('success','Signature Template Updated successfully');
						
					}
					//redirect('crmadmin/product/display_product_list/'.$uri_val.'/'.$rental_id.'/'.$agree_module);
					redirect('crmadmin/product/display_product_list/'.$uri_val);*/	
	...
	public function edit_document(){
			...
			if ($this->checkLogin('CA') == '')
				...
				$dataArr = array('initial_name'=>'','upload_images'=>'','download_name'=>'','sign_status'=>'Failure','preview_images'=>'','image_name'=>'','pageCount'=>'','mail_sent'=>'0');
				$this->product_model->update_details(SIGNTEMPLATE,$dataArr,array('id'=>$sign_id));
...
<!--END OF LOGO-->
 ...
 <!--start of deal-->
	...
	function general_popup_save_options()
		{
	...
	function sendCompleteMailAdmin($rsd_id='')
		{
