...
class Newsletter extends MY_Controller {
     ...
     */
   	public function index(){	
		if ($this->checkLogin('CA') == ''){
			...
			redirect('crmadmin/newsletter/display_subscribers_list');
		}
	}
	...
	public function display_subscribers_list(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/newsletter/display_subscribers',$this->data);
		}
	}
	...
	public function display_newsletter(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/newsletter/display_newsletter',$this->data);
		}
	}
	...
	public function add_newsletter(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->load->view('crmadmin/newsletter/add_newsletter',$this->data);
		}
	}
	...
	public function edit_newsletter_form(){
		if ($this->checkLogin('CA') == ''){
			...
			if ($this->data['user_details']->num_rows() == 1){
				$this->load->view('crmadmin/newsletter/edit_newsletter',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
	...
	public function insertEditNewsletter(){
		if ($this->checkLogin('CA') == ''){
			...
			if (strpos(base_url(),'pleasureriver.com') === false){
				...
				redirect('crmadmin/newsletter/display_newsletter');
			}else {
	...
	public function view_newsletter(){
		if ($this->checkLogin('CA') == ''){
			...
			$this->data['user_details'] = $this->newsletter_model->get_all_details(NEWSLETTER,$condition);
			if ($this->data['user_details']->num_rows() == 1){
				$this->load->view('crmadmin/newsletter/view_newsletter',$this->data);
			}else {
				redirect('deals_crm');
			}
		}
