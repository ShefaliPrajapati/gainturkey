...
if ($app->cfgCon("xi178")) {
    $count=0;
    $results = mysql_query("SELECT * FROM fc_property_reserved_info p LEFT JOIN popup_status ps on ps.reserved_id = p.id WHERE ps.invoice_status = 'complete'");
    if($results){
    while($row = mysql_fetch_assoc($results)){
        $count++;
//        print($count." " . $row["email"]." - ".$row["first_name"]." ".$row["last_name"]." ".$row['prop_address']."<br>");
...
/* for updating existing records
           ...
           foreach($records as $record){
              ...
              $contact = $app->loadCon($record["ContactId"], $returnFields);
...
"_PRUtilities"=>"$".number_format($row["pr_utilities"],2)
           ...
           mysql_query("INSERT INTO infusionsoft (property_id,contact_id,opportunity_id) VALUES (".$row["property_id"].",".$contact_id["ContactId"].",".$opportunity_id.")");
