$(function() {
  ...
  validate:       true
	...
	========================*/
		...
		$('.user_renter :checkbox').iphoneStyle({ checkedLabel: 'User', uncheckedLabel: 'Admin' });
...
function fnFormatDetails ( oTable, nTr )
{
	...
	//alert(aData);
	var newArr = aData.toString().split(',');
	//alert(newArr[1])
	var resId = $('#reserved_Id_'+newArr[1]).val();
	
	var newDataVal = $('#row-'+resId).html();
	var sOut = newDataVal;
	//sOut='';
    return sOut;
}
...
function fnFormatDetails_old ( oTable, nTr )
{
	...
	var newArr = aData.toString().split(',');
	//alert(newArr[1])
	//$('#showId_'+newArr[2]).show();
	var newDataVal = $('#showId_'+newArr[2]).html();
	var sOut = '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">';
    sOut += '<tr><td colspan="11">'+newDataVal+'</td></tr>';
    sOut += '</table>';
	//sOut='';
    return sOut;
}

function fnFormatOpenRow()
{
    var resId = $('#reserved_Id_1').val();
	var newDataVal = $('#row-'+resId).html();
	var sOut = newDataVal;
	return sOut;
}
...
$(function() {
	...
	if( $('#examplerow tbody tr').length >= 1 ){
	
    $('#examplerow thead tr').each( function () {
        this.insertBefore( nCloneTh, this.childNodes[0] );
    } );
     
    $('#examplerow tbody tr').each( function () {
        this.insertBefore(  nCloneTd.cloneNode( true ), this.childNodes[0] );
    } );
     ...
     * Initialse DataTables, with no sorting on the 'details' column
     */
    var oTable = $('#examplerow').dataTable( {
     ...
     * rather it is done here
     */
    $('#examplerow tbody td img').live('click', function () {
	...
	//$('#examplerow tbody tr:first-child').find('td:eq(0) img').trigger('click');

	$('#examplerow tbody tr').find('td:eq(0) img').trigger('click');
	
	}
	
	$("#examplerow_paginate").on("click", "a", function() { 
		var imgstatus = $('#examplerow tbody tr').find('td:eq(0) img').attr('src');
		//alert(imgstatus);
		if(imgstatus=='images/details_open.png'){
		$('#examplerow tbody tr').find('td:eq(0) img').trigger('click');
		}
	});
	
	$("#examplerow_length").on("change", "", function() { 
		$('#examplerow tbody tr').find('td:eq(0) img').trigger('click');
		
	});

	$("#examplerow_filter").on("keyup", "", function() { 
		$('#examplerow tbody tr').find('td:eq(0) img').trigger('click');
...
$(document).ready(function() {
    var table = $('#examplerow1').DataTable( {
    ...
    // Add event listener for opening and closing details
    $('#examplerow1 tbody').on('click', 'td.details-control', function () {
